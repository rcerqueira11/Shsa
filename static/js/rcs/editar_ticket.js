...
$('#modal-verificacion').on('shown.bs.modal', function (e) {
    titulo = 'CONFIRMACIÓN'
	subtitulo = 'Edición de solicitud de inspección'
	mensaje = '¿Esta seguro que desea editar esta solicitud de inspección?'
...
$('#modal-exito').on('shown.bs.modal', function (e) {
	...
	subtitulo = 'Edición de solicitud de inspección'
	mensaje = 'Edición de inspección realizada exitosamente.'
...
$( document ).ready(function() {
	if ($('input[name=radio_titular]:checked', '#form_editar_ticket').val() == undefined){
		$("#radio_si").prop('checked',true)
	}
    valor_radio = $('input[name=radio_titular]:checked', '#form_editar_ticket').val()
...
$('#form_editar_ticket input[name=radio_titular]').on('change', function() {
   valor_radio = $('input[name=radio_titular]:checked', '#form_editar_ticket').val()
    if (valor_radio == "false"){
    	$("#id_trajo_form").show('slide')
    }else{
...
function submit_form_crear_solicitud(){
	...
	var dataForm = $("#form_editar_ticket").serializeArray();
	url = $('#form_editar_ticket').attr('action')
	$.ajax({
	        type: 'POST' ,
